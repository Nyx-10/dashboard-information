      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', background: 'var(--bg-main)' }}>
        ...
        <div style={{ padding: '1.25rem 1.5rem', borderBottom: '1px solid var(--border)', background: 'var(--surface)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
            <img src={chats.find(c => c.id === activeChat)?.avatar} alt="Avatar" style={{ width: '40px', height: '40px', borderRadius: '50%' }} />
            <div>
              <h3 style={{ fontWeight: 600, color: 'var(--text-main)' }}>{chats.find(c => c.id === activeChat)?.name}</h3>
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '1.5rem', display: 'flex', flexDirection: 'column', gap: '1rem' }}>
          ...
          <div style={{ display: 'flex', gap: '1rem', maxWidth: '80%' }}>
            <img src={chats.find(c => c.id === activeChat)?.avatar} alt="Avatar" style={{ width: '32px', height: '32px', borderRadius: '50%', flexShrink: 0 }} />
            <div>
              <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', padding: '1rem', borderRadius: '0 1rem 1rem 1rem', color: 'var(--text-main)' }}>
                {chats.find(c => c.id === activeChat)?.preview}
              </div>
              <span style={{ fontSize: '0.7rem', color: 'var(--text-muted)', marginTop: '0.25rem', display: 'block' }}>10:42 AM</span>
            </div>
          </div>
          
          <div style={{ display: 'flex', gap: '1rem', maxWidth: '80%', alignSelf: 'flex-end', flexDirection: 'row-reverse' }}>
            <div>
              <div style={{ background: 'var(--primary)', color: 'white', padding: '1rem', borderRadius: '1rem 0 1rem 1rem' }}>
                Hi, ya! Boleh saya jumpa awak untuk ambilnya?
              </div>
              <span style={{ fontSize: '0.7rem', color: 'var(--text-muted)', marginTop: '0.25rem', display: 'block', textAlign: 'right' }}>10:45 AM</span>
            </div>
          </div>
        </div>
